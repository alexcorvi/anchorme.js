...
(function(window) {
	 ...
	 * # str : is the string we will process
	 * # seperator : what is considered as a seperator between words?
	 * # options.attributes : the added attributes
	 *
	 **/
		...
		if (anchorme.occurrences(str, "@") == 1) {
			...
			if (EmailAllowed) {
				...
				for (var x = 0; x < howManyTLDsToCheck; x++) {
					...
					if (str.indexOf(d, str.length - d.length) !== -1) {
	...
	anchorme.checks.url = function (str,howManyTLDsToCheck) {
		...
		if(str.indexOf(".") > 0 && (str.indexOf("/") > 3 || str.indexOf("/") < 0)) {
			...
			if (domainAllowed) {
				...
				else if (str.endsWith(".com")) return true;
				else if (str.indexOf(".com/") > -1) return true;
				else {
					for (var x = 0; x < howManyTLDsToCheck; x++) {
						var tld = anchorme.TLDs[x];
						if (str.indexOf(tld, str.length - tld.length) !== -1 || str.indexOf(tld+"/") > -1 || str.indexOf(tld+":") > -1) {
							x = anchorme.TLDs.length;
							return true;
						}
					}
		...
		for (var i = 0; i < splitedArray.length; i++) {
			...
			if(fragment.indexOf(".") > -1) {
				...
				if (allAllowed) {
					...
					else if (anchorme.checks.ip(fragment) && options.ips && fragment.indexOf(".") > 2) {
						isurl = true;
						protocol = options.defaultProtocol;
					}
					
					else if (anchorme.checks.email(fragment,options.TLDs) && options.emails && fragment.indexOf(".") > -1) {
						isurl = true;
						protocol = "mailto:";
					}
