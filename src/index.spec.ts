...
describe("options", function() {
	...
	it("attributes with filters", function() {
		var result = anchorme("www.google.com www.yahoo.com", {
			attributes: [
				function(fragment) {
					if (fragment.raw.indexOf("google") === -1)
	...
	it("truncate", function() {
		...
		var result2 = anchorme(
			"https://github.com/alexcorvi/anchorme.js/blob/gh-pages/test/bench.js",
			{
				truncate: 20
			}
		);
...
describe("Additional functionalities", function() {
	...
	it("URLs validator works with emojis", function() {
		expect(anchorme.validate.url("http://🐌🍏⌚✨😐😍🐸🍑.🍕💩.ws")).toBe(
			true
		);
	});
});
...
describe("Features", function() {
	it("Skipping html: attribute", function() {
		var result = anchorme("<iframe href='https://google.com'></iframe>", {
			list: true
		});
		expect(typeof result).toBe("object");
	...
	it("Skipping html: anchor link", function() {
		var result = anchorme("<a href='https://google.com'>google.com</a>", {
			list: true
		});
		expect(typeof result).toBe("object");
		expect(result.length).toBe(0);
