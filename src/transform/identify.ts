...
export default function(inputArr: Array<string>, options: Options) {
	return inputArr.map((fragment, index) => {
		...
		if (!urlObj) return fragment;
			...
			// make sure it's not inside an HTML attribute
			if (
				(inputArr[index - 1] === "'" || inputArr[index - 1] === '"') &&
				~htmlAttrs.indexOf(inputArr[index - 2])
			) {
				return fragment;
			} else if (
				inputArr[index - 1] === ">" &&
				...
				// it's not already an anchor link tag
				return fragment;
			}
			return urlObj;
		}
