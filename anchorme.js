                ...
                if (nextCheck === "email") {
                    ...
                    if (anchorme.occurrences(sm,"@") == 1) {
                        ...
                        for (var x = 0; x < Ename.length; x++) {
                            var cr = Ename[x];
                            if ("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!#$%&'*+-/=?^_`{|}~.".indexOf(cr) === -1) {x=Ename.length;EmailAllowed = false}
                        }
                        
                        for (var x = 0; x < Eserver.length; x++) {
                            var cr = Eserver[x];
                            if ("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-.:".indexOf(cr) === -1) {x=Eserver.length;EmailAllowed = false}
                        }
                ...
                if (nextCheck === "domains") {
                    ...
                    if (sm.indexOf("/") > 4) {
                        ...
                        for (var x = 0; x < pre_slash.length; x++) {
                            var cr = pre_slash[x];
                            if ("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-.:".indexOf(cr) === -1) {x = pre_slash.length;domainAllowed = false}
                        }
                        
                    } else {
                        for (var x = 0; x < sm.length; x++) {
                            var cr = sm[x];
                            if ("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-.:".indexOf(cr) === -1) {x = sm.length;domainAllowed = false}
                        }                        
                    }
