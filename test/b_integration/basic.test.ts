...
describe("Basic usage of the library", () => {
	describe("Argument types", () => {
		...
		it("Both of the argument have the same result", () => {
			expect(anchorme("string")).toEqual(anchorme({ input: "string" }));
		});
	});
	...
	describe("Basic transformation", () => {
		describe("pure URL string", () => {
			it("Transformation of a URL", () => {
				expect(anchorme("www.google.com")).not.toBe("www.google.com");
				expect(anchorme("www.google.com")).toBe(
					`<a href="http://www.google.com">www.google.com</a>`
				);
			});
			it("Transformation of an email", () => {
				expect(anchorme("alex@arrayy.com")).not.toBe("alex@arrayy.com");
				expect(anchorme("alex@arrayy.com")).toBe(
					`<a href="mailto:alex@arrayy.com">alex@arrayy.com</a>`
				);
			});
			it("Transformation of a file", () => {
				expect(anchorme("file:///C:/Users/alias/file.zip")).not.toBe(
					"file:///C:/Users/alias/file.zip"
				);
		...
		describe("polluted URL string", () => {
			function pollute(string: string) {
				return `here it is:${string}, have a go at it`;
			}
			it("Transformation of a URL", () => {
			...
			it("Transformation of an ip", () => {
				expect(anchorme(pollute("192.168.0.1/admin_panel"))).not.toBe(
					pollute("192.168.0.1/admin_panel")
				);
		...
		const examples = {
			urls: {
				...
				invalid: [
					...
					"a.b@something.com"
					// TODO: "ali.com@facebook.com@yahoo.com"
				]
			},
			emails: {
		...
		describe("Emails", () => {
			...
			describe("invalid emails", () => {
				for (
					...
					it(`Should be invalid ${element}`, () => {
						expect(anchorme.validate.email(element)).toBe(false);
					});
				}
			});
		});
