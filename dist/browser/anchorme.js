(function (global, factory) {
	...
	var domainWithTLD = "([a-z0-9]+(-+[a-z0-9]+)*\\.)+(".concat(dictionary.TLDs, ")");
	var domainWithAnyTLD = "([a-z0-9]+(-+[a-z0-9]+)*\\.)+([a-z0-9][a-z0-9-]{0,".concat(Math.max.apply(commonjsGlobal, dictionary.TLDs.split("|").map(function (x) { return x.length; })) - 2, "}[a-z0-9])");
	...
	var nonLatinMatches = "".concat(fqdn, "((((\\/(([").concat(allowedInPath, "]+(\\/[").concat(allowedInPath).concat(dictionary.nonLatinAlphabetRanges, "]*)*))?)?)((\\?([").concat(allowedInPath, "\\/?]*))?)((\\#([").concat(allowedInPath, "\\/?]*))?))?\\b((([").concat(allowedInPath, "\\/").concat(dictionary.nonLatinAlphabetRanges, "][a-zA-Z\\d\\-_~+=\\/").concat(dictionary.nonLatinAlphabetRanges, "]+)?))+)");
